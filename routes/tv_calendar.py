...
def get_upcoming_episodes():
    ...
    try:
        ...
        # Batch-load watched episodes in one query instead of one lookup
        # per episode (N+1)
        watched_keys = set()
        if upcoming:
            watched_rows = TVEpisodeWatch.query.filter(
                TVEpisodeWatch.user_id == current_user.id,
                TVEpisodeWatch.show_id.in_({ep.show_id for ep in upcoming})
            ).all()
            watched_keys = {
        ...
        for ep in upcoming:
            ...
            watched = (ep.show_id, ep.season_number, ep.episode_number) in watched_keys
