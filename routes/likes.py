...
def check_if_liked(media_id):
    ...
    Also returns the total like count so the frontend can initialize the
    button with a single request instead of calling /likes separately.
    """
    media_type = request.args.get('media_type', 'movie')
    ...
    count = MediaLike.query.filter_by(
        media_id=media_id,
        media_type=media_type
    ).count()
